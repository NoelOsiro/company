...
const BackgroundImage = ({ imageUrl }) => {
    return (
        <div className="bg-no-repeat bg-cover bg-center relative" style={{ backgroundImage: `url(${imageUrl})` }}>
            <div className="absolute bg-gradient-to-r from-gray-900 to-gray-900 opacity-75 inset-0 z-0"></div>
        </div>
    );
};
    ...
    return (
        <div className="flex flex-row  items-center space-x-3 mt-5">
            <a href="https://www.behance.net/ajeeshmon" target="_blank"
                ...
                <svg className="w-4 fill-gray-100" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M8.072 9.301s1.892-.147 1.892-2.459c0-2.315-1.548-3.441-3.51-3.441H0v12.926h6.454s3.941.129 3.941-3.816c-.001-.001.171-3.21-2.323-3.21zM2.844 5.697h3.61s.878 0 .878 1.344c0 1.346-.516 1.541-1.102 1.541H2.844V5.697zm3.427 8.332H2.844v-3.455h3.61s1.308-.018 1.308 1.775c0 1.512-.977 1.669-1.491 1.68zm9.378-7.341c-4.771 0-4.767 4.967-4.767 4.967s-.326 4.941 4.767 4.941c0 0 4.243.254 4.243-3.437H17.71s.072 1.391-1.988 1.391c0 0-2.184.152-2.184-2.25h6.423c.001-.001.709-5.612-4.312-5.612zm1.941 3.886h-4.074s.266-1.992 2.182-1.992 1.892 1.992 1.892 1.992zm.507-6.414H12.98v1.594h5.117V4.16z" /></svg>
            </a>
...
const Testimonial = ({ imageSrc, content, author, role }) => {
    return (
            ...
            <Swiper
                ...
                {testimonials.map((testimonial, index) => (
                    <SwiperSlide key={index}>
                        <Testimonial imageSrc={undefined} content={undefined} author={undefined} role={undefined}/>
                    </SwiperSlide>
                ))}
...
const Testimonials = () => {
    return (
      <div>
        <BackgroundImage imageUrl="your_image_url_here" />
        <Header />
        <div className="grid grid-cols-2 gap-4 items-center z-10">
          {/* ... your other components ... */}
          <SocialLinks />
          <TestimonialSwiper testimonials={yourTestimonialsArray} />
        </div>
      </div>
    );
  };
